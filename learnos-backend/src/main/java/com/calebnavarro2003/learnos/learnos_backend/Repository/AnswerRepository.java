...
public interface AnswerRepository extends JpaRepository<Answer, Integer> {
    ...
    @Query(value = "SELECT COALESCE((SUM(CASE WHEN r.is_correct = 1 THEN 1 ELSE 0 END) * 1.0) / COUNT(r.response_id), 0) "
            +
            "FROM responses r " +
            "JOIN questions q ON r.question_id = q.question_id " +
            "WHERE q.module_id = :moduleId", nativeQuery = true)
    BigDecimal getOverallAccuracyByModuleId(@Param("moduleId") int moduleId);

    @Query(value = "SELECT COUNT(DISTINCT a.user_id) " +
            "FROM answers a " +
            "JOIN questions q ON a.question_id = q.question_id " +
            "WHERE q.module_id = :moduleId", nativeQuery = true)
    Long countDistinctUsersByModuleId(@Param("moduleId") int moduleId);

    @Query(value = "SELECT q.question_id AS questionId, " +
            "COALESCE((SUM(CASE WHEN r.is_correct = 1 THEN 1 ELSE 0 END) * 1.0) / COUNT(r.response_id), 0) AS accuracy "
            +
            "FROM questions q " +
            "LEFT JOIN responses r ON q.question_id = r.question_id " +
            "WHERE q.module_id = :moduleId " +
            "GROUP BY q.question_id", nativeQuery = true)
    List<ModuleQuestionStats> getPerQuestionAccuracyByModuleId(@Param("moduleId") int moduleId);
}